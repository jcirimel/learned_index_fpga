...
class NeuronIO(p_num_inputs: Int, expWidth: Int, sigWidth: Int, relu: Boolean) extends Bundle {
  val inputs = Vec(p_num_inputs, UInt((expWidth + sigWidth + 1).W))
...
class Neuron(p_num_inputs: Int, expWidth: Int, sigWidth: Int, weight: UInt, bias: UInt, relu: Boolean) extends Module {
  ...
  val mul = new MulRecFN(expWidth, sigWidth)
  val add = new AddRecFN(expWidth, sigWidth)
  val sum = UInt()
  ...
  if(p_num_inputs > 1){
    val tree = new AdderTree(p_num_inputs, expWidth, sigWidth)
